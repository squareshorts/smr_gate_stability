...
def run_loso(df_task, dataset_id, variant_label):
    ...
    for mname, mfeats in MODELS.items():
        ...
        for test_sub in subjects:
            ...
            X_tr = scaler.fit_transform(tr[mfeats].fillna(0))
            X_te = scaler.transform(te[mfeats].fillna(0))
...
def main():
    ...
    for variant_label in ["snr_primary", "power_sensitivity"]:
        vb = df_boot_all[df_boot_all["Variant"] == variant_label] if len(df_boot_all) > 0 else pd.DataFrame()
        vl = df_loso_all[df_loso_all["variant"] == variant_label] if len(df_loso_all) > 0 else pd.DataFrame()
    ...
    print("\n✓ run_snr_harmonization.py complete.")
