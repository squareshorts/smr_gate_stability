...
> A repository-level field-deployment reference implementation was added for NF-SQI pseudo-online replay, including rest-baseline calibration, per-window Gate A/B/C decisions, automated tests, and a synthetic 1000-window computational latency benchmark showing mean 1.286 ms per 1-s window. Synthetic pseudo-online demo runs and latency benchmark runs were confirmed. The implementation's logic was rigorously verified via batch-summary reproduction validation, matching the final Gate A/B/C counts exactly against the archived summary CSVs. Raw-window pseudo-online reproduction was not validated because rest calibration windows were not recovered from task EDFs. Feature-level validation was not run because window-feature CSVs were not present in the cleaned repo.
