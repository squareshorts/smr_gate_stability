...
def process_secondary():
    ...
    print(f"Loaded {len(files)} cache files for secondary evaluation.")
    ...
    for f in tqdm(files):
        ...
        df_test = df[df['block'] == 'EEG_MI_test'].copy()
        ...
        task = df_test[df_test['condition'] == 'task'].sort_values('window_start_s').reset_index(drop=True)
        ...
        # Split half calibration on TRAIN rest
        b1 = rest.iloc[:30]
    ...
    pd.DataFrame(q1_rows).to_csv(OUT_DIR / "secondary_q1_cardinality.csv", index=False)
    pd.DataFrame(q2_rows).to_csv(OUT_DIR / "secondary_q2_composition.csv", index=False)
    pd.DataFrame(q3_rows).to_csv(OUT_DIR / "secondary_q3_aggregation.csv", index=False)
    pd.DataFrame(q4_rows).to_csv(OUT_DIR / "secondary_q4_h1.csv", index=False)
    pd.DataFrame(q5_rows).to_csv(OUT_DIR / "secondary_q5_duration.csv", index=False)
    print("Completed secondary evaluations.")
